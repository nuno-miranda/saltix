...
// Ensure persistent session is available before window creation
app.once('ready', () => {
  userSession = session.fromPartition('persist:sapo-mail');
  // Enable cookies and storage persistence
  userSession.cookies.off('changed');
  userSession.cookies.on('changed', () => {});
});
